...
The `opencv-contrib-python` build that gets installed (see Known issues above) requires system-level graphics libraries (`libGL.so.1`, `libgthread-2.0.so.0`) that a minimal Linux container doesn't include by default — without them, `import cv2` fails with `ImportError: libGL.so.1: cannot open shared object file`.

Deploy with the **`Dockerfile`** at the repo root: it installs `libgl1` + `libglib2.0-0` via apt, then `pip install -r requirements.txt`, then runs `streamlit run app.py`. It reads the `PORT` env var (defaulting to `10000`) rather than a fixed port, matching [Render](https://render.com)'s Docker web service convention — Render's free tier (no credit card, 750 hrs/month) is the recommended target; free services sleep after 15 minutes idle with a ~30-60s cold start on the next visit.

Streamlit Community Cloud was tried first (it reads a `packages.txt` file for apt dependencies instead of a Dockerfile) but its build container currently has a broken apt mirror missing `libffi7`, a transitive dependency of `libglib2.0-0` — an infrastructure issue on their end, not fixable from this repo. Hugging Face Spaces' Docker SDK was also considered but requires a paid PRO plan. Both were dropped in favor of the Dockerfile + Render path, which has been built and run locally end-to-end (dependencies install cleanly, all of `test.py` passes, the server boots and responds).
